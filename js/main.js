(() => {
  ...
  const words = ["hangman", "javascript", "pancake", "vampire", "avengers"];

  let guessBox = document.querySelector('#myLetter'),
      wordHint = document.querySelector('.current-word'),
      replay = document.querySelector('.reset-screen button'),
  ...
  function resetGame() {
    ...
    resetScreen.classList.remove('show-piece');
  ...
  function makeAGuess() {
    ...
    if (currentWord.indexOf(currentGuess) < 0) {
      ...
      document.querySelector(`.wrong${wrongAnswers}`).classList.add('show-piece');
